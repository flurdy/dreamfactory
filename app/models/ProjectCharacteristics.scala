...
object ProjectCharacteristics {
  ...
  sealed abstract class Appeal(
      val name: String,
      val alternatives: Set[String] = Set.empty,
      val parentName: String = "appeal"
  ...
  sealed abstract class Complexity(
      val name: String,
      val alternatives: Set[String] = Set.empty,
      val parentName: String = "complexity"
  ) extends Characteristic
  ...
  sealed abstract class Likelihood(
      val name: String,
      val alternatives: Set[String] = Set.empty,
      val parentName: String = "likelihood"
  ) extends Characteristic
  ...
  sealed abstract class DevelopmentStatus(
      val name: String,
      val alternatives: Set[String] = Set.empty,
      val parentName: String = "status.development"
  ) extends Characteristic
  ...
  sealed abstract class ReleaseStatus(
      val name: String,
      val alternatives: Set[String] = Set.empty,
      val parentName: String = "status.release"
  ) extends Characteristic
  ...
  sealed abstract class DeployStatus(
      val name: String,
      val alternatives: Set[String] = Set.empty,
      val parentName: String = "status.deploy"
  ) extends Characteristic
  ...
  case class CharacteristicPossibilities(
      appeals: Set[Appeal],
  ...
  val characteristicPossibilities = CharacteristicPossibilities(
    appeals,
    complexities,
    likelihoods,
    developmentStatuses,
...
case class ProjectCharacteristics(
  ...
  def this(configuration: Configuration) = this(
    appeal = configuration.getOptional[String]("appeal").flatMap(Appeal(_, appeals)),
    complexity = configuration.getOptional[String]("complexity").flatMap(Complexity(_, complexities)),
    likelihood = configuration.getOptional[String]("likelihood").flatMap(Likelihood(_, likelihoods)),
    developmentStatus = configuration
      .getOptional[String]("status.development")
      .flatMap(DevelopmentStatus(_, developmentStatuses)),
    releaseStatus =
      configuration.getOptional[String]("status.release").flatMap(ReleaseStatus(_, releaseStatuses)),
    deployStatus = configuration.getOptional[String]("status.deploy").flatMap(DeployStatus(_, deployStatuses))
  )
  val isEmpty                                           = appeal
    .orElse(complexity)
    .orElse(likelihood)
